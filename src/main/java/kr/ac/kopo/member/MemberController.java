...
public class MemberController {
	...
	/**
	 * 1-1. 로그인 - get memberController에서 get으로 login이 들어오면 뭘 해야하는가 로그인 페이지로 가줘! -> 갈
	 * 때 로그인폼에 입력값이 어떤 값인지 알고 가야한다
	 */
	@GetMapping("/login")
	...
	/**
	 * 1-2. 로그인 - post 로그인버튼을 눌렀을 때 실행될 핸들러 로그인 입력값을 입력하면 -> 그걸 db작업 -> id/pw가 유효하니?
	 * -> yes : board list로 -> no : 다시 login.jsp
	 */
	@PostMapping("/login")
...
//		System.out.println(member);		
		...
		/*
		 * 1.MemberVO의 member가 null값이 아닌 valid한 값이 들어왔는가 확인 -> BindingResult를 가지고 확인
		 * 2-1. null값을 가진 error값이 들어온 경우 forward member/login.jsp ㄱㄱ -> hasError 2-2.
		 * 정상값이 들어온 경우, id/ pw가 유효한지 select문으로 체크 -> else 2-2-1) 유효한 id/pw - db존재하는 애 ->
		 * "welcome! ***님 반갑습니다" 띄워주고 session에 해당 유효한 memberVO를 등록
		 * 
		 * 2-2-2) 유효하지 않을 때 - db존재하지 않는 애 -> 사용자에게 메시지로 id/pw가 유효하지 않으니 다시 입력하세요 라고
		 * 보여주고, 다시 로그인창으로 가야함 -> forward member/login.jsp
		 * 
...
//			System.out.println("테스트" + member);
			...
			if (authMember == null) {
				...
				System.out.println("loginVO : " + session.getAttribute("loginVO"));
				//System.out.println("loginVO : " + dest);
